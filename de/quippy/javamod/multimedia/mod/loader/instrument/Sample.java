...
public class Sample
{
	...
	/**
	 * Fits the loop-data given in instruments loaded These values are often not
	 * correkt
	 * 
	 * @since 27.08.2006
	 * @param modType
	...
	public String toString()
	{
		...
		bf.append('(').append("length:").append(length).append(',').append("fineTune:").append(fineTune).append(',').append("transpose:").append(transpose).append(',').append("baseFrequency:").append(baseFrequency).append(',').append("volume:")
				.append(volume).append(',').append("panning:").append(panning).append(',').append("repeatStart:").append(repeatStart).append(',').append("repeatLength:").append(repeatLength).append(',').append("repeatStop:").append(repeatStop)
				.append(')');
	...
	private int getCubicInterpolated(final int currentSamplePos, final int currentTuningPos)
	{
		...
		final long v1 = (((currentSamplePos - 1) < 0) ? 0L : (long) CubicSpline.lut[poslo] * (long) sample[currentSamplePos - 1]) + ((long) CubicSpline.lut[poslo + 1] * (long) sample[currentSamplePos])
				+ ((long) CubicSpline.lut[poslo + 2] * (long) sample[currentSamplePos + 1]) + ((long) CubicSpline.lut[poslo + 3] * (long) sample[currentSamplePos + 2]);
	...
	private int getFIRInterpolated(final int currentSamplePos, final int currentTuningPos)
	{
		...
		final long v1 = (((currentSamplePos - 3) < 0) ? 0L : (long) WindowedFIR.lut[firidx] * (long) sample[currentSamplePos - 3]) + (((currentSamplePos - 2) < 0) ? 0L : (long) WindowedFIR.lut[firidx + 1] * (long) sample[currentSamplePos - 2])
				+ (((currentSamplePos - 1) < 0) ? 0L : (long) WindowedFIR.lut[firidx + 2] * (long) sample[currentSamplePos - 1]) + ((long) WindowedFIR.lut[firidx + 3] * (long) sample[currentSamplePos]);
		final long v2 = ((long) WindowedFIR.lut[firidx + 4] * (long) sample[currentSamplePos + 1]) + ((long) WindowedFIR.lut[firidx + 5] * (long) sample[currentSamplePos + 2]) + ((long) WindowedFIR.lut[firidx + 6] * (long) sample[currentSamplePos + 3])
	...
	/**
	 * @param sustainLoopStart
	 *            the sustainLoopStart to set
	 */
	...
	/**
	 * @param sustainLoopEnd
	 *            the sustainLoopEnd to set
	 */
