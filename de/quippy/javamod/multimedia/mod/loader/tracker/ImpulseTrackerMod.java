...
public class ImpulseTrackerMod extends ScreamTrackerMod
{
	...
	public void loadModFileInternal(ModfileInputStream inputStream) throws IOException
	{
		...
		// Cmwt: Compatible with tracker with version greater than value. (ie.
		// format version)
		...
		// MV: Mix volume (0->128) During mixing, this value controls the
		// magnitude of the wave being mixed.
		inputStream.skip(1);
		...
		// 4 Byte reserved
		inputStream.skip(4);
		// Chnl Pan: Each byte contains a panning value for a channel. Ranges
		// from
		...
		for (int i = 0; i < getNSamples(); i++)
		{
			...
			/*
			 * Bit 4. On = Use loop Bit 5. On = Use sustain loop Bit 6. On =
			 * Ping Pong loop, Off = Forwards loop Bit 7. On = Ping Pong Sustain
			 * loop, Off = Forwards Sustain loop
			 */
			int loopType = 0;
			...
			if (c4Speed == 0)
				c4Speed = 8363;
			else if (c4Speed < 256) c4Speed = 256;
			...
			if (sampleOffset > 0 && currentSample.length > 0)
			{
				...
				if (CvT == 0xFF && (flags & 0x02) != 0x02)
					loadFlag = Helpers.SM_ADPCM4; // No 16 Bit flag set!
				else
				{
		...
		for (int i = 0; i < getSongLength(); i++)
		{
			if (getArrangement()[i] == 255) // end of Song:
				break;
			else if (getArrangement()[i] < 254 && getArrangement()[i] < getNPattern()) getArrangement()[realLen++] = getArrangement()[i];
		}
