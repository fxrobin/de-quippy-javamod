...
public class ScreamTrackerOldMod extends Module
{
	...
	{
		if (!kennung.equals("!Scream!")) throw new IOException("Mod id: " + kennung + ": this is not a screamtracker mod");
	...
	private PatternElement createNewPatternElement(int pattNum, int row, int channel, int note)
	{
		...
		if (pe.getEffekt() == 0x01) // set Tempo needs correction. Do not ask
									// why!
		{
	...
	public void loadModFileInternal(ModfileInputStream inputStream) throws IOException
	{
		...
		// always space for 128 pattern... With STMs we need to guess the
		// arrangement length
		...
		while (getArrangement()[currentSongLenth - 1] >= getNPattern())
			currentSongLenth--;
