...
public class ScreamTrackerMixer extends BasicModMixer
{
	 ...
	 * @param channel
	 * @param aktMemo
	 * @see de.quippy.javamod.multimedia.mod.mixer.BasicModMixer#initializeMixer(int,
	 *      de.quippy.javamod.multimedia.mod.mixer.BasicModMixer.ChannelMemory)
	 */
	@Override
	...
	protected void resetAllEffects(ChannelMemory aktMemo, PatternElement nextElement, boolean forced)
	{
		...
		{
			if (forced || (nextElement.getEffekt() != 0x08 && nextElement.getEffekt() != 0x0B && nextElement.getEffekt() != 0x15)) // but
																																	// only,
																																	// if
																																	// there
																																	// is
		...
		{
			if (forced || nextElement.getEffekt() != 0x12) // but only, if there
															// is no tremolo
															// following
			{
		...
		{
			if (forced || nextElement.getEffekt() != 0x19) // but only, if there
															// is no panbrello
															// following
			{
	...
	protected void doRowEffects(ChannelMemory aktMemo)
	{
		...
		switch (aktMemo.effekt)
		{
			...
			case 0x04: // Volume Slide
				...
				if (((aktMemo.volumSlideValue >> 4) == 0xF && (aktMemo.volumSlideValue & 0xF) != 0x0) || ((aktMemo.volumSlideValue >> 4) != 0x0 && (aktMemo.volumSlideValue & 0xF) == 0xF) || useFastSlides)
				{
			...
			case 0x05: // Porta Down
				...
				if (indicatorPortaDown == 0xE0 || indicatorPortaDown == 0xF0) // (Extra)
																				// Fine
																				// Porta
																				// Down
				{
			...
			case 0x06: // Porta Up
				...
				if (indicatorPortaUp == 0xE0 || indicatorPortaUp == 0xF0) // Extra
																			// Fine
																			// Porta
																			// Up
				{
			...
			case 0x15: // Fine Vibrato
				// This effect is identicatal to the vibrato, but has a 4x
				// smaller amplitude (more precise).
			...
			case 0x0A: // Arpeggio
				...
				{
					int currentIndex = aktMemo.assignedNoteIndex - 1; // Index
																		// into
																		// noteValues
																		// Table
			...
			case 0x0B: // Vibrato + Volume Slide
				...
				if (aktMemo.effektParam != 0) aktMemo.volumSlideValue = aktMemo.effektParam;
				// Fine Volume Up/Down and FastSlides
				if (((aktMemo.effektParam >> 4) == 0xF && (aktMemo.effektParam & 0xF) != 0) || ((aktMemo.effektParam >> 4) != 0 && (aktMemo.effektParam & 0xF) == 0xF) || useFastSlides)
				{
					doVolumeSlideEffekt(aktMemo);
				}
				break;
			...
			case 0x0F: // Sample Offset
				...
				if ((element.getPeriod() != 0 || element.getNoteIndex() != 0) && aktMemo.currentSample != null)
				{
					if (aktMemo.effektParam != 0)
					{
						...
						if (aktMemo.sampleOffset >= aktMemo.currentSample.length) aktMemo.sampleOffset = aktMemo.currentSample.length - 1;
					}
			...
			case 0x13: // Extended
				...
				switch (aktMemo.effektParam >> 4)
				{
					...
					case 0x9: // TODO Extended Channel Effekt
						switch (effektOpEx)
						{
							case 0x0: // Disable surround for the current
										// channel
								aktMemo.doSurround = false;
								break;
							case 0x1: // Enable surround for the current
										// channel. Note that a panning effect
										// will automatically desactive the
										// surround, unless the 4-way (Quad)
										// surround mode has been activated with
										// the S9B effect.
							...
							case 0x9: // Force reverb for this channel
								break;
							case 0xA: // Select mono surround mode (center
										// channel). This is the default
								break;
							case 0xB: // Select quad surround mode: this allows
										// you to pan in the rear channels,
										// especially useful for 4-speakers
										// playback. Note that S9A and S9B do
										// not activate the surround for the
										// current channel, it is a global
										// setting that will affect the behavior
										// of the surround for all channels. You
										// can enable or disable the surround
										// for individual channels by using the
										// S90 and S91 effects. In quad surround
										// mode, the channel surround will stay
										// active until explicitely disabled by
										// a S90 effect
								break;
							case 0xC: // Select global filter mode (IT
										// compatibility). This is the default,
										// when resonant filters are enabled
										// with a Zxx effect, they will stay
										// active until explicitely disabled by
										// setting the cutoff frequency to the
										// maximum (Z7F), and the resonance to
										// the minimum (Z80).
								break;
							case 0xD: // Select local filter mode (MPT beta
										// compatibility): when this mode is
										// selected, the resonant filter will
										// only affect the current note. It will
										// be deactivated when a new note is
										// being played.
								break;
							case 0xE: // Play forward. You may use this to
										// temporarily force the direction of a
										// bidirectional loop to go forward.
								break;
							case 0xF: // Play backward. The current instrument
										// will be played backwards, or it will
										// temporarily set the direction of a
										// loop to go backward.
								break;
						}
						break;
					case 0xA: // set High Offset
						...
						break;
					case 0xB: // JumpLoop
						// this effekt is simulated through pattern break and
						// position jump
						if (effektOpEx == 0) // Set a marker for loop
						{
							jumpLoopPatternIndex = currentArrangement; // position
																		// jump
																		// index!
						...
						if (jumpLoopRepeatCount == -1) // was not set!
						{
							...
							if (!jumpLoopPositionSet) // if not set, pattern
														// Start is default!
							{
								jumpLoopPatternIndex = currentArrangement; // position
																			// jump
					...
					case 0xE: // Pattern Delay
						if (patternDelayCount <= 0) patternDelayCount = effektOpEx; // if
																					// currently
																					// in
																					// the
																					// patternDelay,
			...
			case 0x14: // set Tempo
				...
				else if (aktMemo.effektParam >> 4 == 1) // 0x1X
					currentBPM += aktMemo.effektParam & 0xF;
				else
			...
			case 0x17: // Global Volume Slide
				...
				if (mod.getModType() != Helpers.MODTYPE_IT) globalVolumSlideValue <<= 1;
	...
	private void doPortaToNoteEffekt(ChannelMemory aktMemo)
	{
		if (aktMemo.portaTargetNotePeriod != aktMemo.currentNotePeriod)
		{
			if (aktMemo.portaTargetNotePeriod < aktMemo.currentNotePeriod)
			{
				...
				if (aktMemo.currentNotePeriod < aktMemo.portaTargetNotePeriod) aktMemo.currentNotePeriod = aktMemo.portaTargetNotePeriod;
			}
			else
			{
				aktMemo.currentNotePeriod += aktMemo.portaNoteStep;
	...
	protected void doVibratoEffekt(ChannelMemory aktMemo, boolean doFineVibrato)
	{
		...
		{
			case 1:
				periodAdd = (Helpers.ModRampDownTable[aktMemo.vibratoTablePos]); // Sawtooth
				break;
			case 2:
				periodAdd = (Helpers.ModSquareTable[aktMemo.vibratoTablePos]); // Squarewave
				break;
			case 3:
				periodAdd = (Helpers.ModRandomTable[aktMemo.vibratoTablePos]); // Random.
				break;
			default:
				periodAdd = (Helpers.ModSinusTable[aktMemo.vibratoTablePos]); // Sinus
				break;
		}
	...
	protected void doTremoloEffekt(ChannelMemory aktMemo)
	{
		...
		{
			case 1:
				volumeAdd = (Helpers.ModRampDownTable[aktMemo.tremoloTablePos]); // Sawtooth
				break;
			case 2:
				volumeAdd = (Helpers.ModSquareTable[aktMemo.tremoloTablePos]); // Squarewave
				break;
			case 3:
				volumeAdd = (Helpers.ModRandomTable[aktMemo.tremoloTablePos]); // Random.
				break;
			default:
				volumeAdd = (Helpers.ModSinusTable[aktMemo.tremoloTablePos]); // Sinus
				break;
		}
	...
	protected void doTickEffekts(ChannelMemory aktMemo)
	{
		...
		switch (aktMemo.effekt)
		{
			case 0x04: // VolumeSlide
				if (!((aktMemo.volumSlideValue >> 4) == 0xF && (aktMemo.volumSlideValue & 0xF) != 0x0) || ((aktMemo.volumSlideValue >> 4) != 0x0 && (aktMemo.volumSlideValue & 0xF) == 0xF))
				{
					doVolumeSlideEffekt(aktMemo);
				}
				break;
			...
			case 0x0C: // Porta to Note + VolumeSlide
				...
				break;
			case 0x0E: // Channel Volume Slide
				if (!((aktMemo.effektParam >> 4) == 0xF && (aktMemo.effektParam & 0xF) != 0x0) || ((aktMemo.effektParam >> 4) != 0x0 && (aktMemo.effektParam & 0xF) == 0xF))
				{
					doChannelVolumeSlideEffekt(aktMemo);
				}
				break;
	...
	/**
	 * @param aktMemo
	 * @return true, if the Effekt and EffektOp indicate a PortaToNoteEffekt AND
	 *         there was a Note set
