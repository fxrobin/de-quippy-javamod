 ...
 *        ------------------------------------------------------------------------------------------------
 *        fir interpolation doc, (derived from "an engineer's guide to fir
 *        digital filters", n.j. loy)
 *
 *        calculate coefficients for ideal lowpass filter (with cutoff = fc in
 *        0..1 (mapped to 0..nyquist)) c[-N..N] = (i==0) ? fc :
 *        sin(fc*pi*i)/(pi*i)
 *
 *        then apply selected window to coefficients c[-N..N] *= w(0..N) with n
 *        in 2*N and w(n) being a window function (see loy)
 *
...
public class WindowedFIR
{
	...
	/**
	 * Constructor for WindowedFIR is not needed!
	 */
	private WindowedFIR()
	{
		super();
	 ...
	 * @return
	 */
	private static double coef(int _PCnr, double _POfs, double _PCut, int _PWidth, int _PType) // float
																								// _PPos,
																								// float
																								// _PFc,
																								// int
	...
	{
		double _LPcllen = (double) (1L << WFIR_FRACBITS); // number of
															// precalculated
															// lines for 0..1
															// (-1..0)
