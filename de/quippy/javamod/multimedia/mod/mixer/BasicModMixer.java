...
public abstract class BasicModMixer
{
	protected class ChannelMemory
	{
		...
		public ChannelMemory()
		{
			...
			actRampVolLeft = actRampVolRight = deltaVolLeft = deltaVolRight = currentVolume = currentSetVolume = channelVolume = channelVolumSlideValue = 0;
			fadeOutVolume = 65536;

			muted = false;
			assignedNotePeriod = currentNotePeriod = currentFinetuneFrequency = currentFineTune = 0;
		...
		/**
		 * DeepCopy by Reflection - iterate through all fields and copy the
		 * values
		 * 
		 * @since 29.03.2010
		 * @param fromMe
	...
	protected boolean useFastSlides; // This is s3m specific and results from a
										// bug in st3
	protected int frequencyTableType; // XM and IT Mods support this! Look at
										// the constants
	...
	// FadeOut
	protected boolean doFadeOut; // means we are in a loop condition and do a
									// fadeout now. 0: deaktivated, 1: fade out,
									// 2: just ignore loop
	...
	/**
	 * Normally you would use the formula (25*samplerate)/(bpm*10) which is
	 * (2.5*sampleRate)/bpm. But (2.5 * sampleRate) is the same as
	 ...
	 * XM_LINEAR_TABLE and XM_AMIGA_TABLE is for XM-Mods, AMIGA_TABLE is for
	 * ProTrackerMods only (XM_AMIGA_TABLE is about the same though) With Mods
	 * the AMIGA_TABLE, IT_AMIGA_TABLE and XM_AMIGA_TABLE result in the
	 * approximate same values, but to be purly compatible and correct, we use
	 * the protracker fintune period tables! The IT_AMIGA_TABLE is for STM and
	 * S3M and IT... Be careful: if XM_* is used, we expect a noteIndex
	 * (0..119), no period!
	 * 
	 * @param aktMemo
	 * @param period
	...
	{
		int noteIndex = period - 1; // Period was both, a mod period or xm/it
									// noteIndex - this changed (look at
									// default!)
		switch (frequencyTableType)
		{
			...
			case Helpers.AMIGA_TABLE:
				return Helpers.protracker_fineTunedPeriods[(aktMemo.currentFineTune >> 4) + 8][period - 25]; // We
																												// have
																												// less
																												// Octaves!
			...
			case Helpers.XM_AMIGA_TABLE:
				...
				final int note = ((noteIndex % 12) << 3) + 8; // !negativ
																// finetune
																// values!
																// -8..+7
																// Therefore add
				...
				if (logIndex < 0)
					logIndex = 0;
				else if (logIndex > 103) logIndex = 103;
			...
			// int logIndex = (note<<3)+fineTune+8; // !negativ finetune values!
			// -8..+7
			// int v1=Helpers.logtab[logIndex];
			...
			default: // Period is not a noteindex - this will never happen, but
						// I once used it with protracker mods
				return (int) ((long) Helpers.BASEFREQUENCY * (long) period / (long) aktMemo.currentFinetuneFrequency);
		}
	}

	/**
	 * Calls getFineTunePeriod(ChannelMemory, int Period) with the aktual Period
	 * assigend. All Effekts changing the period need to call this
	 * 
	 * @param aktMemo
	 * @return
	 */
	protected int getFineTunePeriod(final ChannelMemory aktMemo)
	{
		...
		else if (frequencyTableType == Helpers.IT_LINEAR_TABLE || frequencyTableType == Helpers.STM_S3M_TABLE)
	 ...
	 * playerTuning to be used. I.e. a value like 2, which means every second
	 * sample in the current instrument is to be played. A value of 0.5 means,
	 * every sample is played twice. As we use int-values, this again is
	 * shiftet.
	 * 
	 * @param aktMemo
	 * @param newPeriod
	...
	protected void setNewPlayerTuningFor(final ChannelMemory aktMemo, final int newPeriod)
	{
		...
		else if (frequencyTableType == Helpers.XM_LINEAR_TABLE)
		{
			...
			aktMemo.currentTuning = globalTuning / newPeriod; // in
																// globalTuning,
																// all constant
																// values are
																// already
	...
	/**
	 * Clear all effekts. Sometimes, if Effekts do continue, they are not
	 * stopped.
	 * 
	 * @param aktMemo
	 * @param nextElement
	...
	protected void processEnvelopes(ChannelMemory aktMemo)
	{
		...
		if (currentInstrument != null)
		{
			...
			if (aktMemo.keyOff)
			{
				...
				currentVolume = (currentVolume * aktMemo.fadeOutVolume) >> 16; // max:
																				// 65536
			}
		...
		{
			final int panningPos = (aktMemo.panbrelloTablePos >> 2) & 0x3F; // MPTrack
																			// starts
																			// +0x10
																			// (high
																			// amplitude)
			...
			{
				case 1:
					newPanning = (Helpers.ModRampDownTable[panningPos]); // Sawtooth
					break;
				case 2:
					newPanning = (Helpers.ModSquareTable[panningPos]); // Squarewave
					break;
				case 3:
					newPanning = (Helpers.ModRandomTable[panningPos]); // Random.
					break;
				default:
					newPanning = (Helpers.ModSinusTable[panningPos]); // Sinus
					break;
			}
		...
		currentVolume = (currentVolume * fadeOutValue) >> fadeOutFac; // max:
																		// 255

		if (currentVolume < 0)
			currentVolume = 0;
		else if (currentVolume > Helpers.MAXVOLUME) currentVolume = Helpers.MAXVOLUME;
		...
		if (aktMemo.actVolumeLeft != aktMemo.actRampVolLeft)
		{
			...
			else if (aktMemo.deltaVolLeft != 0) aktMemo.deltaVolLeft /= aktMemo.deltaVolLeft;
		}
		else
		...
		if (aktMemo.actVolumeRight != aktMemo.actRampVolRight)
		{
			...
			else if (aktMemo.deltaVolRight != 0) aktMemo.deltaVolRight /= aktMemo.deltaVolRight;
		}
		else
		...
		if (currentSample != null && currentSample.vibratoDepth > 0 && aktMemo.currentNotePeriod > 0)
		{
			...
			else
			{
				if (!aktMemo.keyOff)
				{
					...
					if ((aktMemo.autoVibratoAmplitude >> 8) > currentSample.vibratoDepth) aktMemo.autoVibratoAmplitude = currentSample.vibratoDepth << 8;
				}
			}
			...
			switch (currentSample.vibratoType & 0x07)
			{
				default:
				case 0:
					periodAdd = Helpers.ft2VibratoTable[aktMemo.autoVibratoTablePos & 0xFF]; // Sine
					break;
				case 1:
					periodAdd = ((aktMemo.autoVibratoTablePos & 0x80) == 0x80) ? +64 : -64; // Square
					break;
				case 2:
					periodAdd = ((0x40 + (aktMemo.autoVibratoTablePos >> 1)) & 0x7f) - 0x40; // Ramp
																								// Up
					break;
				case 3:
					periodAdd = ((0x40 - (aktMemo.autoVibratoTablePos >> 1)) & 0x7F) - 0x40; // Ramp
					...
					break;
				case 4:
					periodAdd = (Helpers.ModRandomTable[aktMemo.autoVibratoTablePos & 0x3F]); // Random.
					break;
			}
	...
	{
		aktMemo.autoVibratoTablePos = aktMemo.autoVibratoAmplitude = aktMemo.currentDirection = aktMemo.currentTuningPos = aktMemo.currentSamplePos = 0;
		aktMemo.volEnvPos = aktMemo.panEnvPos = aktMemo.pitchEnvPos = -1;
		aktMemo.filterOn = aktMemo.instrumentFinished = false;
	}
	...
	// {
	// if (mod.getModType() != Helpers.MODTYPE_IT || !isChannelActive(aktMemo))
	// return;
	// Instrument currentInstrument = aktMemo.assignedInstrument;
	// if (currentInstrument!=null)
	// {
	...
	// {
	// case Helpers.NNA_CUT: newChannel.currentVolume = setNewVolume(newChannel,
	// newChannel.panning, 0); break;
	...
	protected void setNewInstrumentAndPeriod(final ChannelMemory aktMemo)
	{
		...
		// if ((element.getPeriod()>0 || element.getNoteIndex()>0) && !isPorta)
		// // New Note Effekt
		// {
		// checkForNNA(aktMemo);
		// }
		Sample newSample = null;
		// If we have an instrument setting here, set it's volume and panning
		// values
		...
		// if there is a note, we need to calc the new tuning and activate a
		// previous set instrument
		if (element.getPeriod() > 0 || element.getNoteIndex() > 0)
		{
			resetAllEffects(aktMemo, element, true); // Reset Tremolo and such
														// things...
			// Key Off is off per definition
			aktMemo.keyOff = false;
			// We have an instrument assignment, so there was (once) an
			// instrument set!
			if (aktMemo.assignedInstrument != null || aktMemo.assignedInstrumentIndex > 0)
			{
				// Get the correct sample from the mapping table (if not yet set
				// (speedup))
				if (newSample == null)
				{
					if (aktMemo.assignedInstrument != null)
	...
	protected void doRowEvent()
	{
		...
		for (int c = 0; c < nChannels; c++)
		{
			...
			// Now copy the pattern data but remain old values for note and
			// instrument
			...
			else if (element.getPeriod() == Helpers.NOTE_CUT || element.getNoteIndex() == Helpers.NOTE_CUT)
			{
				aktMemo.fadeOutVolume = 0;
			}
			else if (!isNoteDelayEffekt(aktMemo)) // If this is a noteDelay, we
													// lose for now, this is all
													// done later!
			{
	...
	protected boolean doTickEvents()
	{
		...
		if (doFadeOut)
		{
			...
			if (fadeOutValue <= 0) return true; // We did a fadeout and are
												// finished now
		}
		...
		else
		{
			...
			if (currentTick <= 0)
			{
				...
				else if (currentArrangement >= mod.getSongLength())
				{
				...
				else
				{
					// Do the row events
					doRowEvent();
					// and step to the next row... Even if there are no more -
					...
					if (currentRow >= currentPattern.getRowCount() || patternBreakRowIndex != -1 || patternPosJumpPatternIndex != -1)
					{
						...
						if (patternPosJumpPatternIndex != -1)
						{
							...
							if (infinitLoop && doNoLoops == Helpers.PLAYER_LOOP_FADEOUT) doFadeOut = true;
						}
						else
						{
							jumpLoopPositionSet = false; // Stepping to a new
															// Pattern -
															// Position needs
															// new set...
							currentArrangement++;
	...
	/**
	 * Add current speed to samplepos and fit currentSamplePos into loop values
	...
	protected void fitIntoLoops(final ChannelMemory actMemo)
	{
		...
		if (actMemo.currentDirection >= 0)
		{
			...
			if (actMemo.currentTuningPos >= Helpers.SHIFT_ONE)
			{
				...
				{
					if ((ins.loopType & Helpers.LOOP_IS_PINGPONG) == 0) // No
																		// Ping
																		// Pong
					{
						if (actMemo.currentSamplePos >= ins.repeatStop) actMemo.currentSamplePos = ins.repeatStart + ((actMemo.currentSamplePos - ins.repeatStart) % ins.repeatLength);
					}
					else // is PingPong
					{
	...
	private void mixChannelIntoBuffers(final int[] leftBuffer, final int[] rightBuffer, final int startIndex, final int endIndex, final ChannelMemory actMemo)
	{
		...
		{
			// Retrieve the Sampledata for this point (interpolated, if
			// necessary)
			...
			if ((actMemo.deltaVolLeft > 0 && volL > actMemo.actVolumeLeft) || (actMemo.deltaVolLeft < 0 && volL < actMemo.actVolumeLeft))
			{
			...
			if ((actMemo.deltaVolRight > 0 && volR > actMemo.actVolumeRight) || (actMemo.deltaVolRight < 0 && volR < actMemo.actVolumeRight))
			{
	...
	/**
	 * Retrieves Sample Data without manipulating the currentSamplePos and
	 * currentTuningPos and currentDirection (a kind of read ahead)
	 * 
	 * @since 18.06.2006
	 * @param leftBuffer
	 * @param rightBuffer
	...
	/**
	 * Will mix #count 24bit signed samples in stereo into the two buffer. The
	 * buffers will contain 24Bit signed samples.
	 * 
	 * @param leftBuffer
	 * @param rightBuffer
	 * @param count
	...
	public int mixIntoBuffer(final int[] leftBuffer, final int[] rightBuffer, final int count)
	{
		...
		int endIndex = samplePerTicks; // where !will! we be after next mixing
										// (will this still fit?!)
		...
		while (endIndex < maxEndIndex && !modFinished)
		{
			for (int c = 0; c < maxChannels; c++)
			{
				// if (c!=1) continue; //TODO: COMMENT THIS OUT AGAIN IF
				...
				if (!actMemo.muted && isChannelActive(actMemo))
				{
					...
					// and get the ramp data for interweaving, if there is
					// something left
		...
		// currentPatternIndex + "[" + currentRow + "]", ex);
		// }
	}
}
