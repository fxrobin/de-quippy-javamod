...
/**
 * Optimized IIR bandpass filter with only 3 multiplies per sample Used for each
 * band of the graphic equalizer.
 * 
...
public class IIRBandpassFilter extends IIRFilterBase
{
	...
	protected float performFilterCalculation(final float sample, final int channel, final int iIndex, final int jIndex, final int kIndex)
	{
		...
		y[iIndex] = (alpha * ((x[iIndex] = sample) - x[jIndex])) + (gamma * y[kIndex]) - (beta * y[jIndex]);
		return y[iIndex];
	}
}
