...
public class ProTrackerMixer extends BasicModMixer
{
	...
	protected void resetAllEffects(ChannelMemory aktMemo, PatternElement nextElement, boolean forced)
	{
		...
		{
			if (forced || (nextElement.getEffekt() != 0x04 && nextElement.getEffekt() != 0x06)) // but
																								// only,
																								// if
																								// there
																								// is
		...
		{
			if (forced || nextElement.getEffekt() != 0x07) // but only, if there
															// is no tremolo
															// following
			{
		...
		{
			if (forced || nextElement.getEffekt() != 0x22) // but only, if there
															// is no panbrello
															// following
			{
	...
	protected void doRowEffects(ChannelMemory aktMemo)
	{
		...
		switch (aktMemo.effekt)
		{
			...
			case 0x05: // Porta To Note + VolumeSlide
				if (aktMemo.assignedNotePeriod != 0) aktMemo.portaTargetNotePeriod = getFineTunePeriod(aktMemo);
				// With Protracker Mods Porta wihtout Parameter is just Porta,
				// no Vol-Slide
				...
				else
				{
					...
					else if ((aktMemo.effektParam & 0xF) != 0) aktMemo.volumSlideValue = -(aktMemo.effektParam & 0xF);
				}
				break;
			case 0x06: // Vibrato + VolumeSlide
				aktMemo.vibratoOn = true;
				// With Protracker Mods Vibrato without Parameter is just
				// Vibrato, no Vol-Slide
				if (mod.getModType() == Helpers.MODTYPE_MOD && aktMemo.effektParam == 0)
					aktMemo.volumSlideValue = 0;
				else
				{
			...
			case 0x0A: // Volume Slide
				// With Protracker Mods Volumeslide without Parameter is not
				// "old Parameter"
				if (mod.getModType() == Helpers.MODTYPE_MOD && aktMemo.effektParam == 0)
					aktMemo.volumSlideValue = 0;
				else
			...
			case 0x0E:
				...
				switch (aktMemo.effektParam >> 4)
				{
					...
					case 0xE: // Pattern Delay --> # of Rows
						if (patternDelayCount <= 0) patternDelayCount = effektOp; // if
																					// currently
																					// in
																					// the
																					// patternDelay,
			...
			case 0x11: // Global volume slide
				...
				else if ((aktMemo.effektParam & 0xF) != 0) globalVolumSlideValue = -(aktMemo.effektParam & 0xF);
			...
			case 0x19: // Panning slide
				...
				else if ((aktMemo.effektParam & 0xF) != 0) aktMemo.panningSlideValue = -((aktMemo.effektParam & 0xF) << 2);
				break;
	 ...
	 * @param aktMemo
	 * @param newVolume
	 * @see de.quippy.javamod.multimedia.mod.mixer.BasicModMixer#doVolumeColumnEffekt(de.quippy.javamod.multimedia.mod.mixer.BasicModMixer.ChannelMemory,
	 *      int)
	 */
	@Override
