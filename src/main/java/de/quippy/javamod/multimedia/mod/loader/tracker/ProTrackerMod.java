...
public class ProTrackerMod extends Module
{
	...
	/**
	 * Many modfiles are too short or too long. Here we try to find out about
	 * this, as the real saved count of pattern is not saved anywhere.
	 * 
	 * @param fileSize
	 * @return
	 */
	private int calculatePatternCount(int fileSize)
	{
		...
		for (int i = 0; i < getSongLength(); i++)
		{
			...
			if (patternNumber > maxPatternNumber && patternNumber < 0x80) maxPatternNumber = getArrangement()[i];
		}
		...
		if (bytesLeft > 0) // It does not fit!
		{
			...
			{
				// The modfile is too short. The highest pattern is reaching
				// into
	...
	private PatternElement createNewPatternElement(int pattNum, int row, int channel, int note)
	{
		...
		if (getNSamples() > 15)
		{
			pe.setInstrument((((note & 0xF0000000) >> 24) | ((note & 0xF000) >> 12)) & getNSamples()); // &
																										// 0x1F
	...
	public boolean checkLoadingPossible(ModfileInputStream inputStream) throws IOException
	{
		...
		inputStream.seek(0);
		return modID.equals("M.K.") || modID.equals("M!K!") || modID.equals("M&K!") || modID.equals("N.T.") || modID.startsWith("FLT") || modID.startsWith("TDZ") || modID.endsWith("CHN") || modID.equals("CD81") || modID.equals("OKTA")
	...
	public void loadModFileInternal(ModfileInputStream inputStream) throws IOException
	{
		...
		for (int i = 0; i < getNSamples(); i++)
		{
			...
			if (current.length > 0)
			{
				...
				if (repeatStart < repeatStop) current.setLoopType(Helpers.LOOP_ON);
			}
			else
				current.setLoopType(0);
		...
		int bytesLeft = calculatePatternCount((int) inputStream.getLength()); // Get
																				// the
																				// amount
																				// of
																				// pattern
